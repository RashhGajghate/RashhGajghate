...
#define NAME MyCalculator

MODULE_LICENSE("Dual BSD/GPL");
//Function Prototypes
int calc_open(struct inode *inode, struct file *filp);
int calc_release(struct inode *indoe, struct file *filp);
ssize_t calc_write_add(struct file *filp, const char __user *Ubuff, size_t count, loff_t  *offp);
ssize_t calc_read_add(struct file *filp, char __user *Ubuff, size_t count, loff_t  *offp);
ssize_t calc_write_sub(struct file *filp, const char __user *Ubuff, size_t count, loff_t  *offp);
ssize_t calc_read_sub(struct file *filp, char __user *Ubuff, size_t count, loff_t  *offp);
ssize_t calc_write_mul(struct file *filp, const char __user *Ubuff, size_t count, loff_t  *offp);
ssize_t calc_read_mul(struct file *filp, char __user *Ubuff, size_t count, loff_t  *offp);
ssize_t calc_write_div(struct file *filp, const char __user *Ubuff, size_t count, loff_t  *offp);
ssize_t calc_read_div(struct file *filp, char __user *Ubuff, size_t count, loff_t  *offp);
...
//	down(&my_sem);//3.) lock the door by decrementing sema value to 0 , or wait if previously locked
	switch (Minor) {
		...
		case 0:
			new_fops->read = calc_read_add;
			new_fops->write = calc_write_add;
			printk("\nAdd NEW FOPS is %p  AND FILEP is %p ",new_fops, filp);
			break;
		case 1: 
			new_fops->read = calc_read_sub;
			new_fops->write = calc_write_sub;
			printk("\nSub NEW FOPS is %p  AND FILEP is %p ",new_fops, filp);
			break;
		case 2: 
			new_fops->read = calc_read_mul;
			new_fops->write = calc_write_mul;
			printk("\nSub NEW FOPS is %p  AND FILEP is %p ",new_fops, filp);
			break;
		case 3: 
			new_fops->read = calc_read_div;
			new_fops->write = calc_write_div;
			printk("\nSub NEW FOPS is %p  AND FILEP is %p ",new_fops, filp);
			break;
		default:
			printk("\n Driver Not Existant");
...
ssize_t calc_write_add(struct file *filp, const char __user *Ubuff, size_t count, loff_t  *offp)
{
	...
	if(result == 0)
	{
		printk(KERN_ALERT "\nMessage from the user......\n %s\n",Kbuff);
                printk(KERN_ALERT "\n %lu bytes of data Successfully Written.....\n",count);
                retval=count;
		a=(int *)Kbuff;
		b = (int *)&Kbuff[4];
		add_result = *a + *b;
